  ...
  return (
    ...
    >
      ...
      <div className={`ml-auto flex items-center shrink-0 ${isMobile ? "gap-1.5" : "gap-2"}`}>
        {isAuthenticated && (
          <>
            ...
            <button
              ...
              className={`rounded-xl bg-accent hover:bg-accent-hover text-white border-none text-[13px] font-semibold cursor-pointer flex items-center gap-1.5 transition-colors shadow-[0_2px_8px_rgba(0,186,124,0.3)] ${
        ...
        {/* Guest: login/register | Auth: user dropdown */}
        {!isAuthenticated ? (
          <div className="flex items-center gap-2">
            <button
              onClick={() => navigate("/login", { state: { background: location, from: location } })}
              className={`rounded-xl border border-border text-[13px] font-medium text-text-2 cursor-pointer bg-surface hover:bg-surface-2 transition-colors ${isMobile ? "px-3 py-2" : "px-4 py-2"}`}
            >
              Prijavi se
            </button>
            {!isMobile && (
              <button
                onClick={() => navigate("/register", { state: { background: location, from: location } })}
                className="px-4 py-2 rounded-xl bg-accent hover:bg-accent-hover text-white text-[13px] font-semibold border-none cursor-pointer transition-colors shadow-[0_2px_8px_rgba(0,186,124,0.3)]"
              >
                Registruj se
              </button>
            )}
          ...
          <div className="relative" ref={dropdownRef}>
            <button
              ...
              className={`flex items-center gap-2 rounded-xl border border-border cursor-pointer bg-surface hover:bg-surface-2 transition-colors ${
            ...
            {dropdownOpen && (
              <div className="absolute right-0 top-full mt-2 w-48 bg-surface border border-border rounded-2xl shadow-[0_8px_30px_rgba(0,0,0,0.4)] py-1.5 z-50">
                <div className="px-4 py-2 border-b border-surface-2 mb-1">
                  <div className="text-[13px] font-semibold text-text-1">{user?.displayName || user?.username}</div>
                  {user?.displayName && <div className="text-[11px] text-text-3">@{user?.username}</div>}
